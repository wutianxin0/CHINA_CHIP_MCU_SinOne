...
//  最后更正日期:
// 	版本		:
// 	更改记录	:721 rom前256byte禁止查表,因此在编译的时候，设置keil->option for Target->BL51 Locate->Code Range:0x100
//*************************************************************

#include "h\SC91F721_C.h"
#include <intrins.h>
//============================
//模块局部变量定义
//============================
...
unsigned char Addr;
unsigned char code  *POINTER = 0x0f00;								//定义ROM指针地址：0x0f00
...
void IAPWrite(void)
{
 	...
 	IAPADR = Addr;			//写入地址值Addr1(0~255)
 	IAPKEY = 0xf0;			//此值可根据实际调整;保证本条指令执行后到对IAPCTL赋值前时间间隔小于240(0xf0)个系统时钟，否则IAP功能关闭
 	IAPCTL = 0x0a;			//执行IAP写入操作，同时CPU Hold 1ms@16M/8M/4M,0.5ms@1M
 	_nop_();
 	_nop_();
 	_nop_();
 	_nop_();
