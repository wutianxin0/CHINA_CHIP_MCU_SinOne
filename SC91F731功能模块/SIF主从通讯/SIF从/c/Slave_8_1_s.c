...
//	文件名称	: slave_8_1.c
//	作者		: 
//	模块功能	: SIF slave 8+1单字节发送模块
//	局部函数列表:
//  最后更正日期:
// 	版本		:
// 	更改记录	: 从向主发送1Byte的数据，完成发送，切换接收状态，接收主的Stop信号。
//*************************************************************
...
//声明需要调用的全局变量
//=====================================
extern 		unsigned char Sif_Send_dat ;
...
void Sif_Slave_Int() interrupt 5
{
	if(STRIF==1)
	{
		...
		SIFTXD = Sif_Send_dat;		//准备发送的数据
		SIF_SET_SEND; 			   //slave切换为发送状态，发送1byte的数据
	}
	else if(TXIF==1)
	{
		TXIF = 0;					//数据发送完成,中断标志清0
		SIF_SET_RECE;              //发送完成1byte,slave切换为接收状态
	}
	else if(STPIF==1)
	{
